...
create table if not exists events (
  ...
  journey_completed text[] not null default '{}',
  given_at timestamptz,
  background_until date,
...
create unique index if not exists events_source_event_id_unique on events(source_event_id) where source_event_id is not null;
create unique index if not exists gift_history_event_id_unique on gift_history(event_id) where event_id is not null;

create table if not exists recipient_ai_usage (
...
-- Server-owned recipient AI allowance and atomic occasion completion.
...
drop policy if exists "Users read their own gift history" on gift_history;
create policy "Users read their own gift history" on gift_history
  for select to authenticated
  using (created_by = (auth.jwt() ->> 'email'));
